                  ...
                  <div class="form-group">
                    ...
                    <?php if(isset($_GET['id'])){?>
                     ...
                     <optgroup label="Admin">
                      ...
                      <?php
                                  $sql2 = "SELECT * FROM school_admin WHERE school_reg= '$reg'";
                                  $result2=mysql_query($sql2);
                                  $row2 = mysql_fetch_array( $result2 );
                                  ?>
                                    <option value="<?php echo $row2['user_name'] ?>" ><?php echo $row2['user_name']."   (".$row2['first_name']." ".$row2['last_name'].")"; ?></option>
                                    </optgroup>
                                    <?php
                                  $sql2 = "SELECT * FROM employee WHERE school_reg= '$reg'";
                                  $result2=mysql_query($sql2);
                                  ?> <optgroup label="Employee"> <?php
                                  if (mysql_num_rows($result2) > 0) {
                                  // output data of each row
                                  while($row2 = mysql_fetch_array( $result2 ))
                                   { if($id==$row2['employee_name']){
                                          continue;
                                        }
                                    ?>
                                    <option value="<?php echo $row2['employee_name'] ?>" ><?php echo $row2['employee_name']."   (".$row2['first_name']." ".$row2['last_name'].")"; ?></option>
                                    <?php
                                    }
                                  }
                                  else {
                                  ...
                                  while($row2 = mysql_fetch_array( $result2 ))
                                   { ?>
                                    <option value="<?php echo $row2['student_name'] ?>" ><?php echo $row2['student_name']."   (".$row2['first_name']." ".$row2['last_name'].")"; ?></option>
                                    <?php
                                    }
                                  }
                                  else {
                                  ...
                                  while($row2 = mysql_fetch_array( $result2 ))
                                   { ?>
                                    <option value="<?php echo $row2['parent_name'] ?>" ><?php echo $row2['parent_name']."   (".$row2['first_name']." ".$row2['last_name'].")"; ?></option>
                                    <?php
                                    }
                                  }
                                  else {
